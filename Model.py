...
class SVDMode:
    ...
    def SVDtrain(self, trainset):
        ...
        for epoch in range(self.epochs):
            ...
            for uid, iid, rating in tqdm(trainset.get_all_ratings()):
                ...
                for i in range(self.features):
                    ...
                    self.UserCharacteristicMatrix[uid, i] += lr * (
                                eui * self.ItemCharacteristicMatrix[iid, i] - regularization * temp)
                    self.ItemCharacteristicMatrix[iid, i] += lr * (
                                eui * temp - regularization * self.ItemCharacteristicMatrix[iid, i])
            x.append(epoch + 1)
        ...
        plt.ylabel("RMSE")
        plt.plot(x, y)
        plt.savefig('Result/train_RMSE.png')
        plt.show()
    ...
    def SVDpredict(self, testset):
        ...
        for user, item in tqdm(testset):
            ...
            if self.trainset.exist_user(user) and self.trainset.exist_item(item):
                ...
                score += self.UserBias[uid] + self.ItemBias[iid] + np.dot(self.ItemCharacteristicMatrix[iid],
                                                                          self.UserCharacteristicMatrix[uid])
